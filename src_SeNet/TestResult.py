...
from TestDataLoader import testsetFolder
...
resnet_model = torch.load('./model/senet15410.pkl')
...
def get_loaders():
    ...
    testset = testsetFolder(transform=transform_test)
...
def predict():
    ...
    predicted = []
    filenames = []
    for index, (filename, inputs) in enumerate(testloader):
        inputs = inputs.cuda(args.GPU_ids)
...
df.to_csv('./predictions/prediction', index=False)
